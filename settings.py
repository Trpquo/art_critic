...
def get_class_names_per_axis():
    class_names = {}
    for axis in categories:
        class_names[axis] = list(categories[axis].keys())
    return class_names

def get_style_names_per_axis():
    style_names = {}
    for axis in categories:
        style_names[axis] = []
        for category in categories[axis]:
            style_names[axis] = style_names[axis] + categories[axis][category]
    return style_names

def get_all_selected_styles():
    all_styles = []
    for style_group in get_style_names_per_axis().values():
        all_styles = all_styles + style_group
    return all_styles